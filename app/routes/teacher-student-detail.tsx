  ...
  return (
    <div className="text-right">
      <div className="mb-8 pb-6 border-b border-slate-200">
        ...
        {student.class && (
          <Link to={`/teacher/classes/${student.class.id}`} className="text-indigo-600 hover:text-indigo-800 text-sm font-bold mt-1 inline-block">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
          <p className="text-2xl font-black text-indigo-600">{analysis.avgScore}</p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50">
          <h3 className="text-lg font-bold text-slate-900">سجل الاختبارات ({student.submissions.length})</h3>
        </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {student.submissions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 font-bold text-indigo-600">{sub.score}/{sub.quiz.totalMarks}</td>
                    <td className="px-6 py-4 text-slate-500">{sub.submittedAt ? new Date(sub.submittedAt).toLocaleDateString("ar-EG") : "لم يُقدم"}</td>
                    <td className="px-6 py-4">
                      <Link to={`/teacher/quizzes/${sub.quiz.id}/submissions/${sub.id}`} className="text-indigo-600 hover:text-indigo-800 text-xs font-bold">عرض</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
