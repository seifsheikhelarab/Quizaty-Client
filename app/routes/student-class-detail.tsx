...
export default function StudentClassDetail({ loaderData }: Route.ComponentProps) {
  ...
  const handleLeaveClass = async () => {
    ...
    try {
      ...
      setFeedbackMessage(err.message || "فشل في مغادرة الفصل");
    ...
    <div className="text-right">
      ...
      <div className="mb-8 pb-6 border-b border-slate-200">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-success-100 text-success-800 mb-2">فصل دراسي</span>
          ...
          <div className="flex gap-3">
            {classData.teacher.phone && (
              <a
                ...
                className="inline-flex items-center px-4 py-2 border border-success-200 text-sm font-bold rounded-lg text-success-700 bg-success-50 hover:bg-success-100 transition-colors"
              >
            ...
            <button
              ...
              className="inline-flex items-center px-4 py-2 border border-danger-200 text-sm font-bold rounded-lg text-danger-700 bg-danger-50 hover:bg-danger-100 transition-colors cursor-pointer"
            >
              مغادرة الفصل
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          {activeQuizzes.length > 0 && (
            <div className="bg-white rounded-2xl border border-success-200 p-6">
              <h3 className="text-lg font-bold text-success-800 mb-4 flex items-center gap-2">
                <span className="w-2 h-2 bg-success-500 rounded-full animate-pulse"></span>
                اختبارات مفتوحة الآن
              </h3>
              <div className="space-y-3">
                {activeQuizzes.map(q => (
                  <Link key={q.id} to={`/student/quizzes/${q.id}`} className="block p-4 rounded-xl bg-success-50 hover:bg-success-100 transition-colors">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-900">{q.title}</span>
                      <span className="px-2 py-1 bg-success-500 text-white text-xs font-bold rounded-lg">مفتوح</span>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {upcomingQuizzes.length > 0 && (
            <div className="bg-white rounded-2xl border border-warning-200 p-6">
              <h3 className="text-lg font-bold text-warning-800 mb-4">اختبارات قادمة</h3>
              <div className="space-y-3">
                {upcomingQuizzes.map(q => (
                  <div key={q.id} className="block p-4 rounded-xl bg-warning-50">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-900">{q.title}</span>
      ...
      <ConfirmDialog
        ...
        title="مغادرة الفصل"
        description={`سيتم إخراجك من فصل "${classData.name}" ولن تتمكن من العودة إليه إلا عبر دعوة جديدة من المعلم.`}
        confirmLabel="مغادرة الفصل"
        busy={isLeavingClass}
        onCancel={() => {
          if (!isLeavingClass) setShowLeaveDialog(false);
        }}
