  ...
  return (
    <div className="max-w-4xl mx-auto text-right">
      <div className="mb-8">
        ...
        <div>
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-black bg-indigo-50 text-indigo-700 mb-3 border border-indigo-100">{quiz.title}</span>
        ...
        <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-500 font-medium flex-row-reverse">
          {quiz.showResults ? (
            <>
              <div className="flex items-center bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm flex-row-reverse">
            ...
            <div className="flex items-center bg-amber-50 px-4 py-2 rounded-xl border border-amber-100 shadow-sm flex-row-reverse">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden min-h-[400px] flex flex-col items-center justify-center p-8 text-center">
        {quiz.showResults ? (
          <div className="w-full text-right">
            ...
            <div className="space-y-8">
              {quiz.questions.map((q: any, index: number) => {
                ...
                if (studentAnswer !== -1) {
                  badgeColor = isCorrect ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800";
                  badgeText = isCorrect ? "صحيحة" : "خاطئة";
                }

                return (
                  <div key={q.id} className="border border-slate-100 rounded-2xl p-6 bg-white shadow-sm relative overflow-hidden group">
                    ...
                    <div className="pr-3">
                      <div className="flex justify-between items-start mb-6 flex-row-reverse">
                        <h4 className="text-lg font-bold text-slate-900 leading-snug pl-4 text-right flex-1">
                      ...
                      {q.imageUrl && (
                        <div className="mb-6"><img src={q.imageUrl} alt="سؤال" className="max-h-64 rounded-xl border border-slate-200" /></div>
                      )}
                      <div className="space-y-3 mt-4">
                        {q.options.map((opt: string, optIndex: number) => {
                          const isSelected = studentAnswer === optIndex;
                          const isActualCorrect = q.correctOption === optIndex;
                          let optClass = "border-slate-100 text-slate-600 bg-white";
                          let label = "";
                          ...
                          return (
                            <div key={optIndex} className={`flex items-center justify-between p-4 rounded-xl border text-sm flex-row-reverse ${optClass}`}>
                              <span className="font-medium text-right">{opt}</span>
                              {label && <span className="text-xs font-black shrink-0">{label}</span>}
                            </div>
                          );
                        })}
                      </div>
          ...
          <div className="py-12">
            <div className="w-20 h-20 bg-amber-100 rounded-3xl flex items-center justify-center mx-auto mb-6 transform -rotate-6">
      ...
      <div className="mt-12">
        <Link to="/student/quizzes" className="w-full inline-block text-center bg-slate-900 text-white font-black py-5 rounded-2xl hover:bg-slate-800 transition-all shadow-xl shadow-slate-200">
