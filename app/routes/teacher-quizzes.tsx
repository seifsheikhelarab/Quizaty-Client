    ...
    <div className="text-right space-y-10">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          ...
          <div className="flex flex-col sm:flex-row-reverse items-center gap-4">
            ...
            <Link
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center px-8 py-4 bg-primary-600 text-white text-base font-black rounded-2xl shadow-xl shadow-primary-100 hover:bg-primary-700 hover:scale-[1.02] active:scale-95 transition-all animate-glow"
            >
      ...
      {quizzes.length === 0 ? (
        <div className="bg-white rounded-[4rem] border border-slate-200 p-16 md:p-24 text-center opacity-0 animate-reveal-up delay-200">
          <div className="w-24 h-24 bg-primary-50 rounded-4xl flex items-center justify-center mx-auto mb-8 transform rotate-6 animate-float">
            <svg className="w-12 h-12 text-primary-600 -rotate-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Link to="/teacher/quizzes/create" className="inline-flex items-center justify-center px-10 py-4 text-lg font-black rounded-2xl text-white bg-primary-600 hover:bg-primary-700 shadow-2xl transition-all hover:-translate-y-1">
              أنشئ أول اختبار لك
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-8">
          {filtered.map((quiz, i) => {
            ...
            return (
              <div 
                key={quiz.id} 
                className={`group bg-white rounded-[2.5rem] border border-slate-200 p-8 flex flex-col hover:shadow-2xl hover:border-primary-200 transition-all duration-500 opacity-0 animate-reveal-up ${
              ...
              >
                ...
                <Link to={`/teacher/quizzes/${quiz.id}`} className="block mb-6 text-right">
                  <h3 className="text-2xl font-black text-slate-900 group-hover:text-primary-600 transition-colors line-clamp-2 leading-tight">{quiz.title}</h3>
                ...
                <div className="mt-auto pt-8 border-t border-slate-100 flex items-center justify-between flex-row-reverse">
                  <div className="flex gap-3">
                    <Link 
                      ...
                      className="inline-flex items-center justify-center px-5 py-2.5 bg-primary-600 text-white text-xs font-black rounded-xl hover:bg-primary-700 transition-all active:scale-95"
                    >
                      التفاصيل
                    </Link>
                    <button
